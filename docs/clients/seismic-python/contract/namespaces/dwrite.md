...
5. Returns a `DebugWriteResult` containing:
   * **plaintext\_tx** — Transaction before encryption
   * **shielded\_tx** — Transaction with encrypted calldata
   * **tx\_hash** — Transaction hash from broadcast
...
Returns a [`DebugWriteResult`](../../api-reference/signature/debug-write-result.md) with three fields:
...
* [PlaintextTx](../../api-reference/signature/plaintext-tx.md) — Plaintext transaction structure
* [UnsignedSeismicTx](../../api-reference/signature/unsigned-seismic-tx.md) — Shielded transaction structure
