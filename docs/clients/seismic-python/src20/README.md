...
{% hint style="warning" %}
`balanceOf()` takes **no arguments**. The contract uses `msg.sender` internally, so you must use `.read` (a [signed read](../../../gitbook/client-libraries/seismic-python/src20/signed-reads.md)) — not `.tread`. A plain `eth_call` zeros out the `from` field, which means the contract sees the zero address as the sender and returns its balance — which is almost certainly zero.
{% endhint %}
