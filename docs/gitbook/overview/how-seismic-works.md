...
* **Low-entropy private storage values**: Shielded storage hides values from RPC reads, but each value is still committed into the storage Merkle trie at a publicly known slot. A value with little entropy — a flag, a small enum — can be recovered by guessing candidates and checking them against the public trie commitment, without ever reading the slot. See [Footguns](../seismic-solidity/footguns.md#low-entropy-shielded-storage-values) for details and mitigations.
