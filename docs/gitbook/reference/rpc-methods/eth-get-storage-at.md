...
{% hint style="warning" %}
If you need to read shielded data, use a [signed read](../seismic-transaction.md#signed-reads) via `eth_call` with a Seismic transaction. The contract itself must expose a getter function for the shielded value.
{% endhint %}
